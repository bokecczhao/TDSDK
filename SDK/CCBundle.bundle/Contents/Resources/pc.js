...
!(function (window, document) {
  var isOpenWaterMark = Utils.getURLParameter('waterMark') == 1
  var roomid = 0
  var accountid = 0
  var type = 0
  var recordId = 0
  ...
  PC.prototype.showWhiteBorad = function (d) {
    ...
    if (!wb) {
      ...
      try {
        window.webkit.messageHandlers.dpwhiteBoardError.postMessage({})
      } catch (e) {
  ...
  PC.prototype.showJPG = function (d) {
    ...
    function creatImage(url) {
      ...
      if (isOpenWaterMark) {
        ...
        var imgUrl = url + '?x-oss-process=image/auto-orient,1/resize,m_lfit,w_' + w + '/quality,q_50'
    ...
    function imageLoadError(e) {
      ...
      try {
        window.webkit.messageHandlers.dpImageLoadError.postMessage({})
      } catch (e) {
  ...
  PC.prototype.dpBufferingStart = function () {
    isBuffering = true
  }
  PC.prototype.dpBufferingEnd = function () {
    isBuffering = false
  }

  PC.prototype.showAnimation = function (d) {
    console.log('翻页是否loading', isBuffering)
    // 画板展示的宽和高
    var dpDisplayedWidth = window.innerWidth
    var dpDisplayedHeight = window.innerHeight
